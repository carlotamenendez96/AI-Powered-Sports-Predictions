...
**Implication**: an edge has to come from information the price does not already contain (injuries/lineups/weather/motivation — cf. `scripts/d4_injuries/`), not from re-weighting features derived from the price. The conviction lane (+5.5% ROI), which ignores EV entirely, is currently the only lane carrying its own weight; value is −6.0% and model −3.6%.
