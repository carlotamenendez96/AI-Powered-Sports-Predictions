...
| H2H feature experiment | `python3 scripts/experiment_h2h.py` — A/B the head-to-head block against a **shuffled placebo** of the same width; writes `output/experiments/h2h_<ts>.{json,txt}` |
| H2H post-hoc calibrator test | `python3 scripts/experiment_h2h_calibrator.py` — can a finished probability be corrected by H2H? Cross-fitted coefficients vs price / model / both; writes `output/experiments/h2h_calibrator_<ts>.{json,txt}` |
| Settled-bet performance report | `python3 scripts/report_performance.py [--since YYYY-MM-DD] [--min-bets N]` — ROI by lane / market / **odds band** / league / month, every row with a bootstrap 95% CI; writes `output/performance/<ts>.{json,txt}` |
| Edge check (is the model beating the price?) | `python3 scripts/run_edge_check.py [--since YYYY-MM-DD]` — also the **Edge Watch** pane's ▶ button at the bottom of `/football/`; writes `output/edge_checks/<ts>.{json,txt}` |
...
3. `feature_engineering.py` produces: implied probabilities (`IP_H/D/A`), ELO, rolling form (points/GF/GA/OU/shots/corners), season-to-date PPG and attack/defense strength vs league average (`H_ppg`/`A_ppg`/`H_att`/`A_att`/`H_def`/`A_def`/`ppg_diff`/`abs_ppg_diff`/`att_def_diff`), and home-only / away-only specific form.

   **Head-to-head (2026-09-18)**: nine `h2h_*` columns describe the *pairing* rather than the two teams separately — `h2h_n/pts/gd/goals/draw_rate/ou_rate` over the last `H2H_WINDOW` (5) meetings plus `h2h_venue_n/pts/goals` restricted to meetings at this venue. Like `FORM_WINDOWS`, the constant and the `h2h_features()` helper in `feature_engineering.py` are shared by both sides: `_add_h2h_features` builds them in training (one date-ordered pass; same-day fixtures scored as a block so they can't see each other) and `predict_matches.get_h2h_stats` mirrors them at serve time (verified: 1,080 values across both paths, 0 mismatches). Averages are **NaN, not 0**, for a first-ever meeting. **The columns are built but NOT trained on** — they measured flat both as features and as a post-hoc calibrator, so they are deliberately absent from `common_features` (which stays at 56). They remain on the engineered frame so the experiments stay runnable. See "No measured edge over the market" below.

   **Rolling-form windows (2026-09-13)**: `FORM_WINDOWS = ((5, ''), (10, '_l10'), (15, '_l15'))` at the top of `feature_engineering.py` is **the single source of truth for both sides of the pipeline** — training builds these in `_calculate_rolling_multi`, and `predict_matches.get_team_stats` reads the same constant to derive serve-time form from the same `MatchHistory` corpus. Add a window there and both sides change together; that is the point, since a window present in training but missing at serve time is precisely the train/serve skew the constant exists to prevent (verified: 450 (team, date, window, stat) values compared across the two paths, 0 mismatches). The 5-match window keeps the **unsuffixed** column names (`H_form_pts`, …) because every trained model, feature manifest and heuristic references them. `_calculate_rolling_multi` computes all windows in **one pass** — the expensive part is the per-row `team_matches[team][date < d]` filter, so slicing once and taking a different `.tail(n)` per window makes three windows cost about what one did; `_calculate_rolling(df, window, suffix)` remains as a thin single-window wrapper and is byte-identical to the pre-refactor output. Model features add `pts/gf/ga` at L10 and L15 for both sides plus `H_form_trend`/`A_form_trend` (L5 minus L15 — recent form against the team's own baseline), 14 new columns, `common_features` 42 → 56. (The H2H block above would make 65, but it is not trained on.) **Shots/corners are deliberately L5-only**: they are absent for 23 of 44 leagues and the missingness is perfectly league-clustered (zero partial leagues), so a longer window buys no coverage — measured NaN rates go 0.8% → 1.6% → 2.4% in a data-rich league and stay flat at 99.5% in a data-poor one. See "No measured edge over the market" for the outcome — the features are used by the model but do **not** produce an edge.
4. `train_model.py` trains XGBoost models: multi-class 1X2, binary draw, and Poisson O/U 2.5. Time-series 5-fold CV. Models saved to `models/xgb_model_{1x2,draw,ou}.json`, with feature lists in `models/features_*.json`. **Note (2026-05-19)**: the binary draw model is **trained but no longer used at inference**. It used to be averaged into the 1X2 multi-class output as a "second opinion" on draws, but investigation showed it was acting as an implicit base-rate regularizer — exactly what the per-league Platt calibrator (Phase C4) handles in a more principled, league-specific way. The model file is preserved on disk for backward compatibility and possible future reactivation, but `predict_matches.py` no longer loads or calls it. See FOOTBALL_NEXT_STEPS D0 for full reasoning. `tune_model.py` runs a 6-stage stepwise hyperparameter search and writes `models/best_params_*.json`. **Calibration refresh** (Phase C5): `bin/retrain_pipeline.sh` automatically chains `scripts/run_fit_calibration.py` (refit Platt per league × market, ~6 min) and `scripts/run_validate_calibration.py` (chronological-holdout validate, auto-filter entries that regress >5%, ~6 min) after the model retrain, so `data_sets/league_calibration.json` stays in sync with the model. Both calibration steps are non-fatal — if either fails, the prior calibration file remains in place. **Since 2026-09-13 both steps enforce discrimination guards**: `fit_league_calibrators` drops any league/market whose worst Platt slope is below `MIN_PLATT_SLOPE` (0.3) or whose argmax accuracy falls, recording why in the file's `rejected_at_fit`; the holdout validator carries `acc_delta` / `auc_delta` / `worst_slope` and `improved` now requires a Brier win *without* discrimination loss. Brier/ECE/log-loss alone cannot see this — a calibrator that collapses to the league base rate trades resolution for calibration and still improves all three, which is exactly how the bad 2026-08-25 fit passed. Note the calibration output is currently **not consumed by football** (see step 5). **Model registry / estimator seam (2026-06-23)**: the model *family* for each football head is decoupled from the pipeline behind `ml_project/model_registry.py`. `REGISTRY[market][family]` (markets `1x2`/`ou`/`draw`) returns a `ModelSpec` whose `build()` yields a fresh estimator implementing a uniform contract — `predict_proba` for `1x2`/`draw`, `predict`→Poisson λ for `ou`. `train_model.py`, `predict_matches.py`, and `scripts/benchmark_models.py` all go through this seam and never construct XGBoost directly. Families: 1X2 `xgboost`/`logreg`/`rf`; O/U `xgboost`/`poisson_glm`; draw `xgboost`/`logreg`. Pick a family per head via `ModelTrainer(..., model_family=, ou_family=, draw_family=)` or the env vars `MODEL_FAMILY_1X2` / `MODEL_FAMILY_OU` / `MODEL_FAMILY_DRAW` (each defaults to `xgboost`, which is **byte-identical** to the pre-seam path — verified by diffing model files + prediction CSVs). Training writes a `models/model_meta_<market>.json` sidecar (`{family, artifact, market}`); XGBoost saves its native JSON, other families `joblib.dump` to `models/sk_model_<market>_<family>.joblib`. `predict_matches.py` loads each head via `load_1x2_model` / `load_ou_model`, which read the sidecar and fall back to the legacy `xgb_model_<market>.json` when none exists (so a pre-seam checkout is unchanged). `league_cat` is XGBoost-only categorical; non-XGBoost families drop it from the feature list, and `known_leagues` is decoded from whichever served head is XGBoost (1X2 preferred, O/U fallback) so a mixed-family setup still handles categories correctly. **Caveat**: swapping the production model invalidates `league_calibration.json` (fit against the deployed model's OOF preds) — roll real model changes through `bin/retrain_pipeline.sh` so calibration refits in the same run. `scripts/benchmark_models.py` runs a cross-model value-bet backtest (5-fold TimeSeriesSplit, EV-gated flat stakes) with `implied`/`prior` baselines; writes `output/benchmarks/<ts>.{json,txt}`.
5. `predict_matches.MatchPredictor` (invoked by `run_predictions.sh`) loads models, fetches/derives features for the scraped upcoming-matches JSON, and writes `output/predictions_YYYY-MM-DD.csv`. Season-to-date PPG/strength at inference is mirrored from the standings JSON via `HeuristicAdjuster.get_team_strength` to avoid train/serve skew. **Calibration step (Phase C4)**: between the raw model output and the heuristic adjuster, `MatchPredictor` calls `apply_platt_1x2 / apply_platt_ou` (`ml_project/calibration/apply.py`) which looks up per-league Platt scaling parameters from `data_sets/league_calibration.json`. Leagues without a calibrator (or when the per-sport `use_league_calibration` flag in `betting_config.json` is `false`) pass raw probs through unchanged. `predictions_*.csv` carries both: `Home Win %` / `Draw %` / `Away Win %` / `Over %` / `Under %` are the calibrated-then-heuristically-adjusted final values, and `Home Win % (raw)` / `Draw % (raw)` / `Away Win % (raw)` / `Over % (raw)` / `Under % (raw)` are the model's pre-calibration output for audit. Two extra columns — `Cal 1X2 Source` and `Cal O/U Source` — show whether the calibrator came from full-features OOF (`full`), minimal-features OOF (`minimal`), or no calibrator was applied (empty). Flashscore-format league names map to calibration keys via `ml_project/calibration/league_aliases.py`; add new entries there to enable calibration for newly-scraped leagues.
   ...
   **⚠️ DISABLED for football since 2026-09-13** — `use_league_calibration` is `false` in the football block of `betting_config.json`, so both `Cal 1X2 Source` and `Cal O/U Source` are **empty on every row** and the `(raw)` columns equal the pre-heuristic values. The 2026-08-25 fit shipped 27/37 leagues with at least one **negative** Platt slope, which inverts the model's ordering: it pushed probability *up* exactly where the model was least confident. Over 659 settled matches this moved the 1X2 pick on 22% of them (133 of 144 flips were away→home) and dropped pick accuracy from 49.5% to 41.1%; the 146 settled bets placed on a flipped pick returned −34.0% ROI against −19.6% on the bets it left alone. Platt alone caused 142 of the 144 flips. O/U calibration measured as neutral and was switched off with it only because the flag covers both markets. Guards added the same day (`MIN_PLATT_SLOPE`, plus accuracy/AUC gates in `fit.py` / `validate.py`) reject this class of calibrator, but replaying the shipped file through them rejects **37/37** 1X2 entries — a refit produces a near-empty file. Per-league one-vs-rest Platt on ~650 rows has no ordering signal to work with; restoring calibration means **pooling the fit** (one global calibrator with a per-league intercept offset). Re-enable the flag only once a fit survives the guards.
...
**Auto-cashout (functionality test, 2026-05-26)**: a dashboard **"Auto-cashout" checkbox** arms **server-side autonomous** auto-cashout — it does NOT depend on a browser tab. The checkbox POSTs to `POST /football/auto_cashout/arm` (`on=1|0`), persisting `output/auto_cashout_armed.json`. A daemon thread `_auto_cashout_scheduler()` (started in `__main__`, so it runs under `bin/manage_server.sh`) refreshes Flashscore (reusing `_launch_live_refresh()`) then runs `_run_auto_cashout_sweep()` every `_AUTO_CASHOUT_INTERVAL_S` (10 min) while armed. The sweep evaluates every OPEN bet on a live match and fires the lane-cascading `VirtualBettingBackend.execute_cashout` on any whose decision is non-`hold`. `POST /football/auto_cashout` runs one sweep on demand (diagnostic). The browser "Auto 10m" checkbox is now just a UI-refresh convenience, independent of cashout. The decision is `_cashout_decision()` in `web_ui/app.py` — the **single source of truth** shared with the display badge. It's driven by `adj_prob` (the LiveAdjuster's synthesis of the live stats — score, minute, xG, shots, possession, dominance, red cards — so the decision *is* stats-based without re-reading raw stats): `lock_in` if in profit AND (`adj_prob ≥ _AUTO_CASHOUT_LOCK_IN_PROB` (0.85, near-certain, odds-independent) OR `fair/stake ≥ _AUTO_CASHOUT_LOCK_IN_RATIO` (1.5, big unrealized profit)); `stop_loss` if `adj_prob < _AUTO_CASHOUT_STOP_LOSS_PROB` (0.20); else `hold`. A minute floor (`_AUTO_CASHOUT_MIN_MINUTE`, 30) suppresses both before in-play stats are reliable. (The probability branch exists because `fair/stake = odds×adj_prob×0.95` is odds-capped — a ≥1.5× rule alone can never lock in a bet at odds <~1.58, even at near-certain win.) **Prices at the synthetic estimate** (`get_cashout_amount` = `stake × odds × adj_prob × 0.95`), so it tests cashout TIMING/MECHANISM, not real bookmaker economics — **virtual money only, no real bet placed**. Every evaluation (fired, would-have-fired, or held) is appended to `output/auto_cashout_log.jsonl` for audit/threshold-tuning. The auto loop is Flashscore-only (never triggers Pamestoixima). See FOOTBALL_NEXT_STEPS phase 7c.

**Shadow mode (2026-09-10)**: a second **"Shadow" checkbox** next to Auto-cashout controls whether armed sweeps actually fire. In shadow, the sweep evaluates and logs exactly as before but calls no `execute_cashout` — bets stay `OPEN`, no bankroll moves. State lives alongside arming in `output/auto_cashout_armed.json` as `{"armed": bool, "shadow": bool}`; **a missing `shadow` key defaults to `true`**, so a sweep never starts moving money on its own. Rationale: cashout is priced at fair value (`stake × odds × adj_prob × 0.95`), which is EV-negative by exactly that haircut — it reduces variance, not expected loss (measured: −€26.12 over 64 executed cashouts, of which −€25.44 was `stop_loss`; `scripts/evaluate_auto_cashout.py`). Shadow accrues decision data at zero cost. Log entries carry `shadow`, `would_fire`, and — recorded but never used to price or decide — `bookmaker_offer` / `bookmaker_age_s` from `_load_bookmaker_offers`, which is the data needed to test whether `adj_prob` beats the real live price. Because nothing fires, a shadowed bet stays `OPEN` and is re-logged every sweep, so **scoring takes the FIRST non-`hold` entry per `bet_id`** as the would-have-fired moment; `evaluate_auto_cashout.py` dedupes accordingly and reports executed and shadow decisions in separate sections. Both `on` and `shadow` are **optional** on `POST /football/auto_cashout/arm` and an omitted field is preserved — each checkbox posts only the field it owns, so a stale tab can't silently re-enable real firing. `POST /football/auto_cashout` accepts `shadow=0|1` to override for one sweep. **Startup disarms real firing only** (`__main__`): shadow arming survives restarts on purpose — the unconditional disarm is what stalled live collection between 2026-06-14 and 2026-09-10.

**Per-lane bankroll impact (2026-09-10)**: `evaluate_auto_cashout.py` reports how each lane's bankroll moved (executed) or *would have* moved (shadow) — the "what if live cashout were on" question. Necessary because the audit log stores only ONE representative bet per `bet_id` while `execute_cashout` cascades across every lane holding it, so **the log's headline understates total money movement**: over the 64 historical cashouts the representative-only net Δ is −€26.12, but the true across-lane total is **−€48.62** (value −€29.26, conviction −€12.12, model −€7.24). `_load_lane_bets()` recovers each lane's own stake/odds/`cashout_amount` from the slips; `_lane_impact()` prefers the **recorded** `cashout_amount` as ground truth and only scales the representative's amount by stake share when none exists (the shadow case). Scaling is exact for shadow — a cascade prices every lane at one instant off the same `adj_prob`, and price is linear in stake — but **invalid for the historical executed rows**, whose lanes were cashed at different times pre-cascade (e.g. `2026-05-20:freiburg:aston_villa:o_u:over_2_5`: value cashed 22:58, model 08:23 next morning, ratios 1.2578 vs 1.0859). The report prints per-lane `recorded`/`scaled` counts so the basis is always visible.

**Live-adjuster tracing (2026-09-10)**: `LiveAdjuster.adjust_probabilities` / `adjust_ou_probabilities` take an optional `trace=dict`; when passed it records the branch taken, the dominance score, and the probability vector after each of the four 1X2 stages (`time_decay` → `dominance` → `sterile_possession` → `red_card`), or the pace/blend internals for O/U. Results are byte-identical with and without a trace, and existing callers (`backtest/simulator.py`) are unaffected. `run_live_analysis.py` passes traces and persists them to `live_history` as `trace_1x2` / `trace_ou`, alongside `priors_synthesized` (previously present in the UI record but **dropped** from the persisted history, so lower-quality synthesized-prior snapshots were indistinguishable in stored data). Without the trace only the before/after was stored and per-stage attribution was unrecoverable. Note `LiveAdjuster` emits no log strings of its own — the `adj_logs` in `predictions_*.csv` come from the separate `HeuristicAdjuster` in the prediction path.

**Deployment / auto-start (2026-09-10)**: the Flask UI (and with it the auto-cashout scheduler thread that drives live shadow collection) starts at **user login** via a LaunchAgent at `~/Library/LaunchAgents/com.sportspredictor.webui.plist` — `RunAtLoad`, `KeepAlive=false` so `bin/manage_server.sh stop` still wins and manual control is never fought (trade-off: a crash is not auto-recovered). It is a LaunchAgent, so it runs at login, **not** at power-on, and nothing starts before a user logs in.

**The project must NOT live under `~/Documents`, `~/Desktop` or `~/Downloads`.** Those are TCC-protected and a launchd-spawned process cannot read files there without Full Disk Access — the job dies before serving. The failure is confusing because it surfaces in three different ways as you work around it: launchd's own `WorkingDirectory` and `StandardOutPath` handling fail first with `EX_CONFIG` (78) since launchd performs those in its own context, and only once both are moved into the child does the real error appear (python exiting 1, unable to read `web_ui/app.py`). Probing from a launchd job shows the exact boundary: listing the directory is allowed and writing is allowed, but reading a file returns `Operation not permitted`. The project was moved from `~/Documents/projects/sports_predictor` to `~/projects/sports_predictor` for this reason; the venv was recreated because 42 scripts in `venv/bin` carry an absolute shebang. If it is ever moved again, recreate the venv and update the plist's three absolute paths.

**Live-window gating (2026-09-10)**: the scheduler used to scrape Flashscore every 10 min around the clock while armed. `_live_window_active()` now gates each tick on whether any OPEN bet's match is plausibly in play — from its kickoff (`date`, `"YYYY-MM-DD HH:MM"`) until `_LIVE_MATCH_WINDOW_S` (150 min: 90' + half-time + stoppage + buffer) after it. On a typical slate this cuts ~144 scrapes/day to ~59, because a day leaves a long dead stretch (2026-09-10: an 11.8-hour gap between the early matches ending ~08:00 and the evening kickoffs at 19:50) that a "start at the earliest kickoff" rule would scrape straight through. **Fails OPEN**: OPEN bets with no parseable kickoff return `True`, since silently suppressing collection is worse than a wasted scrape. The scheduler logs only on window transitions, not every tick. The manual `POST /football/auto_cashout` is ungated — it's a diagnostic and always runs.
...
- `/football/auto_wager` reads the latest `output/predictions_*.csv` and builds three parallel slips:
  - **Value lane** — EV-gated, Option B sizing (`value_bankroll × EV × Conf × stake_multiplier`), per-bet cap 3%. **Kept deliberately as an experiment (2026-09-13), not as a strategy that is believed to work** — see "No measured edge over the market" below.
...
- `data_sets/standings/` — JSON files written by `standings_spider` (current standings + form tables). **Known limitation (verified 2026-09-13)**: the `last_5_*` / `last_10_*` form tables are **not actually windowed**. Flashscore's form URLs put the window and venue in a hash fragment (`…/#/<id>/form/home/10/`), and the SPA does not re-render on hash navigation — probing `form/overall/5`, `form/overall/10`, `form/home/5` and `form/home/10` with `page.goto` returns byte-identical tables *even after a forced reload*. The spider reaches home/away by **clicking** the in-page tab, which does drive the router (so `last_5_home` ≠ `last_5_away` is real), but the tab's href always targets the 5-match table — so `last_10_matches_home.json` and `last_10_matches_away.json` are byte-identical copies of their `last_5` counterparts. Only `last_5_matches_overall` / `last_10_matches_overall` come from distinct start URLs, and even those carry identical per-team rows. Also note `matches_played` and `goals` in a form table are **season** figures, not window figures — only `points` and `last_5_results` are window-scoped. Do not treat any `last_10_*` file as ten-match data. Deeper history for features should come from `data_sets/MatchHistory/` (the same corpus training uses), not from these tables.
...
- `output/matches_<date>.json` — **pre-match slate** from the date-based scrape, including the 1X2 odds. No longer overwritten by verification (see below), so it stays available as the market baseline for every settled day.
- `output/results_<date>.json` — verification-scrape output (final scores). Written by `bin/run_verification.sh` since 2026-09-13; before that, results were written back over `matches_<date>.json` with `-O`, which destroyed the odds on every verified day — verification-mode parsing reads scores and does not re-read odds. Both `evaluate_predictions.py` and `resolve_daily_bets.py` take `--results` explicitly. Days verified before this change have no recoverable odds.
...
- `output/performance/<timestamp>.{json,txt}` — settled-bet performance reports from `scripts/report_performance.py`. Answers "where has the money gone", as opposed to `edge_checks/`' "is the EV signal real". Settled (`WON`/`LOST`) bets only. **Every row carries a bootstrap 95% CI, and that is the point of the script**: per-bet returns are heavy-tailed, so a 40-bet league's ROI is close to meaningless as a point estimate. First full run (2026-09-18, 2,606 bets, €8,301 staked, −3.6% ROI overall, CI [−8.4%, +1.1%]) established two things worth not re-deriving. **(a) The per-league table is mostly noise**: of 32 leagues with ≥20 bets, 3 had a CI excluding zero against ~1.6 expected by chance and none of the 3 were positive; league ROI also correlates with the league's *odds mix* (Spearman −0.43, p=0.015), so part of the spread is (b) in disguise. The report prints this check and a multiple-comparisons caution under the table — do not size bets off it. **(b) The odds ladder is real and monotone**: model confidence runs 61.6% → 45.0% across bands where the actual hit rate runs 78.7% → 4.3%, while the market's implied probability tracks reality to within ~1pp at every band below 5.0. That is the probability compression in "No measured edge over the market", measured on settled money rather than OOF. Lanes: value −8.9%, conviction +4.9%, model −3.8% — note **conviction's CI includes zero**, so it is a promising point estimate, not an established edge.
- `output/edge_checks/<timestamp>.{json,txt}` — edge-check reports from `scripts/run_edge_check.py`, surfaced by the **Edge Watch** pane at the bottom of the football dashboard (`_latest_edge_summary` + `POST /football/run_edge_check` in `app.py`, same subprocess+`TASKS` pattern as the cashout backtest). Three questions per run: does claimed EV predict realised return (Spearman ρ over every settled bet — the headline, ρ ≤ 0 means the EV gate is selecting for the model's own error); did disabling calibration help (before/after split on `CALIBRATION_OFF_DATE`); and does the model beat backing the favourite on the same matches. That last one is **only scoreable for days whose pre-match odds survived**, i.e. 2026-09-13 onward — see `results_<date>.json` below — so the pane shows a scoreable-days counter that fills in one day at a time. Settled (`WON`/`LOST`) bets only; `CASHED_OUT`/`VOID` are excluded because their P/L reflects a cashout or refund rather than whether the pick was right.
...
## No measured edge over the market (2026-09-13)

Two independent measurements agree that the football model adds **nothing** on top of the bookmaker price. Read this before investing in EV-gating, calibration, or stake-sizing work — none of those address the cause.

1. **Live settled bets** (2,354 bets, €7,598 staked, −2.6% ROI). Spearman correlation between claimed EV and realised per-euro return is **−0.038** — zero, if anything backwards. Bets the model called *negative* EV returned **+4.0%**; bets it called EV > 0.50 returned **−25.0%**. The effect survives controlling for odds: within every odds band above 1.5, `EV>0` did worse than `EV<=0`. Overall Brier: model **0.2393** vs market **0.2226** — the price is better calibrated, and in every disagreement band the market's error is smaller.
2. **`scripts/experiment_odds_free.py`** (OOF, 11,735 rows, run 2026-09-10). Stacking `logit(p_model) + logit(p_market)`, the nats added over the devigged price are **+0.00000** (with odds) and **+0.00006** (odds-free). The fitted weight on the model is **negative in three of four arms**. Dropping `B365*`/`IP_*` did not free a suppressed signal — the odds-free arm is slightly worse.

**Mechanism**: the model's probabilities are compressed toward 0.5. At odds > 3.0 it says 0.456 where reality is 0.226. Since `EV = conf × odds − 1`, that compression *manufactures* EV precisely where the model is most wrong, so the EV gate selects for the model's own error. With ~96% of picks matching the bookmaker favourite (measured on the 2026-09-13 slate), it is largely reading the price back.

**Tested and ruled out — deeper form history (2026-09-13)**: the obvious next lever was longer rolling windows, on the theory that a 5-match sample is too noisy. Added L10/L15 form + trend (14 features, see feature engineering step 3) and retrained. The model **does** use them — 13 of 14 appear in the 1X2 booster at **15.9% of total gain**, with `H_form_gf_l15` ranked #9 overall, ahead of `abs_elo_diff`. It changed nothing that matters:

| metric | before (42 feats) | after (56 feats) |
| --- | --- | --- |
| nats added over market, 1X2 with_odds | +0.00000 | +0.00001 |
| nats added over market, 1X2 odds_free | +0.00006 | +0.00007 |
| nats added over market, O/U with_odds | +0.00000 | +0.00000 |
| blend weight on model (1X2 with_odds) | −1% | **−8%** |
| model logloss (1X2 with_odds) | 1.0048 | **1.0052** |

The optimal weight on the model given the price moved *further negative*, and logloss got fractionally worse. On the live slate it shifted 1 pick in 80 (mean probability shift 0.0069). Better form measurement makes the model marginally better at **reproducing** the price, not at disagreeing with it profitably — unsurprising in hindsight, since form is computed from results the market has also seen. **Do not retry this with more windows or different stats**; the constraint is the information source, not the resolution.

**Tested and ruled out — head-to-head history (2026-09-18)**: the other obvious lever was the record between the two specific teams — every other feature describes them independently. Nine `h2h_*` columns were added (see feature engineering step 3) and A/B'd by `scripts/experiment_h2h.py`.

The raw effect is real: over rows with a full 5-meeting sample, actual over-2.5 rate runs **45.0% → 55.1%** monotonically across `h2h_ou_rate` bands. It is simply not incremental to the price.

| head | arm | nats added over market | OOF Brier | Brier vs base |
| --- | --- | ---: | ---: | ---: |
...
Retraining the real `ModelTrainer` both ways on the identical frame and predicting the 2026-09-17 slate: **0 of 12 picks changed** on either market (mean |Δ Home Win %| 0.0125, |Δ Over %| 0.0158), and CV accuracy went 0.5053 → 0.5040 on 1X2. (A first pass comparing against the *deployed* model showed 1 pick moving per market — that was the deployed model's age, not the feature. Compare like with like.)

**Two methodological findings from this run that apply to every future feature test — reuse the placebo arm:**

1. **A width-matched placebo is mandatory at these effect sizes.** The tuned booster runs `colsample_bytree=0.6`, so going 59 → 68 columns changes which features each tree can pick from; that perturbation alone moves Brier ~0.03%, the same order as the effect under test. The placebo arm carries the nine columns **row-permuted as a block** (same width, same marginals, same NaN pattern, no link to the fixture). On 1X2 it scored **+0.00003 nats against the real block's +0.00000** — the shuffled columns beat the real ones. Judging "h2h vs base" alone would have read as a small O/U win.
2. **Gain share is not evidence that a feature carries information.** The H2H block takes 8.2% of 1X2 gain and 11.9% of O/U gain (`h2h_goals` at rank #6 of 70) — but the **shuffled** block takes **8.9%** and **10.1%**. XGBoost spends splits on a high-cardinality numeric column whether or not it knows anything. This **retroactively weakens the L10/L15 gain-share claim above** ("13 of 14 in the booster at 15.9% of gain") — that figure was never compared against a shuffled control and this result suggests a control would absorb much of it. The nats/Brier verdict for L10/L15 stands; only the gain-share supporting evidence is in question.

**Also tested as a post-prediction calibrator** (`scripts/experiment_h2h_calibrator.py`), because a booster seeing 65 columns through `colsample_bytree=0.6`/`max_depth=3`/`gamma=5` can drown a signal a direct one-parameter fit would catch. Unregularised logistic, coefficient per 1 SD, 5-fold cross-fitted:

| market | term | on PRICE | on MODEL | on MODEL+PRICE |
| --- | --- | ---: | ---: | ---: |
| O/U | `h2h_ou_rate` | z **+0.8** | z **+2.6** | z **+0.8** |
| O/U | `h2h_goals` | z +0.5 | z **+2.7** | z +0.5 |
| 1X2 home | `h2h_pts` | z +0.8 | z +1.5 | z +0.8 |
| 1X2 draw | `h2h_draw_rate` | z −1.1 | z −0.8 | z −1.1 |

A post-hoc H2H correction **does** measurably improve the model's own O/U probabilities (z +2.6 / +2.7 — the only thing in this investigation to clear a significance bar). But against the price the same terms are flat, and adding the price collapses the term from z +2.6 to z +0.8: the calibrator's whole contribution is pushing the model back toward the price it already had. Best case +0.00020 nats against vig worth 0.02–0.05. This also pre-empts the elaborate variants (per-league H2H calibrator, shrinkage to a league base rate, an H7 rule in `heuristic_adjuster.py`) — all are restricted forms of a fit whose coefficient is already zero against the price.

**Rolled back from `common_features` (2026-09-18)**, unlike the L10/L15 precedent: flat measurement, and nine columns of `colsample_bytree` perturbation the placebo showed is not free. Kept: the builder, the serve-time mirror, `ModelTrainer._training_rows`, both experiment scripts. `_training_rows` exists because of this feature — all three heads used to `dropna` on the full feature list, which with NaN-bearing H2H columns silently discarded **14.7% of the training set**; it is inert now but is the guard that makes re-adding safe, and applies to any future feature that is legitimately missing rather than absent by accident.

**Implication**: an edge has to come from information the price does not already contain (injuries/lineups/weather/motivation — cf. `scripts/d4_injuries/`), not from re-weighting features derived from the price. Longer form windows and head-to-head history are both now measured instances of the latter, not predictions. The conviction lane (+5.5% ROI), which ignores EV entirely, is currently the only lane carrying its own weight; value is −6.0% and model −3.6%.
