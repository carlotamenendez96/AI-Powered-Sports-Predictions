...
| 8d  | **Pace/momentum cashout signal — slope, not level** | 🔴 tested 2026-09-10 — **negative on real trajectories, not promoted** | **Idea:** every shipped cashout rule reads a *level* — `adj_prob` now (`stop_loss`), drift-from-kickoff (`late_drift`), unrealized profit ratio (`lock_in_profit`). None read the *slope*: two matches at identical cumulative xG / `adj_prob` at minute 70 can be on opposite trajectories (one team surging, one fading), and a single snapshot can't tell them apart. Pace is the derivative the decision is currently blind to — orthogonal signal, not a re-pack of `adj_prob`. **Stub shipped (no production wiring):** (1) the backtest simulator now threads a backward-compatible `ctx['history']` — the prior per-snapshot `{minute, score, stats, adj_prob, fair_cashout}` of *this* bet's walk (`ml_project/backtest/simulator.py`); pre-existing rules ignore it. (2) `momentum_fade` rule (`ml_project/backtest/rules.py`, registered in `RULES`): while in profit and past minute 30, fire (lock in) if the selection's `adj_prob` slope over the recent ~8-min window ≤ `-0.010`/min — i.e. live pace turning against us before the static bars trip. Thresholds are module constants (`_MOM_MIN_MINUTE`, `_MOM_MIN_PROFIT_RATIO`, `_MOM_FADE_VELOCITY`, `_MOM_WINDOW_MIN`), **first-cut guesses, NOT tuned.** Because it's in `RULES`, the weekly `run_backtest.py` already scores its Δ automatically — it starts accruing evidence with zero further work. **Known confound (documented in the rule):** slope on `adj_prob` carries a *time-decay* artifact near full time (the LiveAdjuster bleeds probability toward the clock regardless of momentum). The intended next iteration computes the slope on a **raw, decay-free pace stat** — opponent xG-per-minute (and a 1H-rate vs 2H-so-far-rate variant for second-half matches). **Validation gate (same bar as 8b/8c):** score in the weekly backtest over accrued `live_history_*.jsonl`; promote into `_cashout_decision()` — as a bounded `trend` nudge on the `lock_in`/`stop_loss` thresholds, not an override — only once it beats `null`/`late_drift`/`stop_loss` by a meaningful margin **with a stable sign across runs** (the Δ-stability rule under "Weekly backtest re-run"). **Why not now:** data is thin (~8 days, mostly 2–3 matches/day, ~40 multi-snapshot trajectories) and the per-window xG is lumpy → tuning today would curve-fit noise. **Kick-off:** keep collecting `live_history` (auto-cashout sweep / Auto-10m already append it); revisit at the **2026-06-08** weekly-backtest checkpoint and each run after — when `momentum_fade`'s Δ is positive and sign-stable on real trajectories, tune the constants, then wire into `_cashout_decision()`. **RESULT (2026-09-10): gate tried, failed.** First scoring on real trajectories gives `momentum_fade` **−5.63 (model) / −28.56 (value) / 0.00 (conviction)** — negative where it fires, on only **4 triggers** across 69 trajectories (3.6% / 18.2% trigger rate). Not promoted into `_cashout_decision()`. **Also found: it had been scoring nothing at all.** The stub's claim that "because it's in `RULES`, the weekly `run_backtest.py` already scores its Δ automatically — zero further work" was **false** — `--rules` defaults to `null,lock_in_profit,stop_loss,late_drift` (`scripts/run_backtest.py:138`), which omits it. The default now includes `momentum_fade` so the claim holds going forward. Given the whole-family sign flip above (every level-based rule also went negative on real data), the honest read is that the problem is **not** that the signal is a level rather than a slope — it's that fair-value cashout is EV-negative by construction. Revisit only if the real arm ever shows a positive rule to improve on. |
...
**Always pass an explicit `--start` covering the `live_history_*.jsonl` dates.** `--start` defaults to today−30, and trajectory coverage is sparse and clustered (2026-05-18 → 06-14, then 09-10), so the default window routinely contains *zero* real trajectories and the run silently reports `real=0` — pure synthetic, which the 2026-09-10 run showed is sign-inverted from reality. To score the real arm:

```bash
PYTHONPATH="$(pwd):$(pwd)/ml_project" python3 scripts/run_backtest.py \
  --start 2026-05-18 --end 2026-06-14 --data real \
...
### 2026-09-10 run — the synthetic/real sign flip (read this before tuning any cashout rule)

The two 2026-09-10 rows above are the **same window and the same slips**, differing only in trajectory source. **Every `late_drift` and `stop_loss` cell flips sign**, across all three lanes (model arm: `late_drift` +59.72 → −13.77, `stop_loss` +75.38 → −10.06; conviction: +11.67 → −9.82 and +9.21 → −8.22). On real trajectories only **one of fifteen** rule×lane cells is meaningfully positive (`lock_in_profit/model`, +4.70). `null` Δ = 0.00 in both arms, so the harness self-validates.

This is the Δ-stability rule firing as loudly as it can, and it **retires the "both Δ-positive" justification** recorded for `stop_loss`/`late_drift` in 8b — that finding came from synthetic trajectories. It also independently corroborates the live measurement: real auto-cashout ran **−€48.62** across lanes (`scripts/evaluate_auto_cashout.py`, per-lane impact). Two unrelated methods now agree cashout as implemented is EV-negative; the synthetic harness was the lone dissenter.
...
- [ ] **D2 — Feature engineering pass.** Three tested-and-rolled-back plus two tested-and-kept-but-flat so far (see `FEATURE_ENGINEERING_IDEAS.md` for full results). **Emerging finding: cheap features the betting market already prices give XGBoost no lift** — the model leans on the B365 implied-probability features (`IP_H/D/A`), which already encode form, rest, congestion, etc.
  ...
  - ~~**§1.6 Head-to-head history**~~ — tested 2026-09-18, **no edge**. Sparsity was never the problem (89% of rows have a prior meeting, 56% have five) and the univariate signal is real (over-2.5 rate 45.0% → 55.1% across `h2h_ou_rate` bands), but incremental nats over the market are +0.00000 and OOF Brier moves −0.001% (1X2) / +0.054% (O/U) against a ≥1% bar. **Also tested as a post-prediction calibrator**, not just as a feature: a direct one-parameter fit on the finished probability does improve the model's own O/U numbers (`h2h_ou_rate` z +2.6), but the same term is flat against the price (z +0.8) and collapses to z +0.8 once the price is in the equation — the correction only pushes the model back toward the price. That rules out the per-league / shrinkage / heuristic-H7 variants too, since all are restricted forms of the same fit. **Rolled back from `common_features`** (back to 56); builder, serve mirror and both experiment scripts kept, so re-adding is one line. `scripts/experiment_h2h.py`, `scripts/experiment_h2h_calibrator.py`.
  - ⚠️ **Three methodology upgrades came out of §1.6 — apply them to every future feature test.** (a) A/B against a **width-matched shuffled placebo**, not against base: `colsample_bytree=0.6` means adding columns perturbs every tree's choices by ~0.03% Brier on its own, and the H2H placebo actually *outscored* the real block on 1X2. (b) **Stop citing gain share as evidence** — the shuffled block took 8.9% of 1X2 gain vs the real block's 8.2%. (c) **Test the post-hoc form separately from the feature form** — they are different functional questions and the h2h answer differed between them (z +2.6 vs nothing), even though the verdict landed the same. All three are in `scripts/experiment_h2h.py` / `scripts/experiment_h2h_calibrator.py` and cheap to reuse; the latter caches base-arm OOF at `output/experiments/_oof_base.pkl` for any other post-hoc question.
  - **Reconsider the remaining Tier-1 candidates before spending more days**: §1.4 promoted-team and §1.7 goal-difference, manager-change are ALL things the market prices → likely similarly flat (§1.6 is now a measured instance, not a prediction). The real headroom is (a) signals priced *inefficiently* (key-player injuries, lineup leaks — overlaps D4) or (b) reducing odds-dependence so the model learns fundamentals (overlaps D3). Don't grind more date/form features expecting Brier wins.
...
### Infrastructure enabling the pivots

- [x] **D-infra — Model registry / estimator seam.** Done 2026-06-23. The model *family* per football head is decoupled from the pipeline behind `ml_project/model_registry.py` (`REGISTRY[market][family]` → `ModelSpec.build()`; markets `1x2`/`ou`/`draw`). `train_model.py`, `predict_matches.py`, and the new `scripts/benchmark_models.py` all go through a uniform `fit`/`predict_proba`(/`predict`→λ) contract instead of constructing XGBoost directly. Family is selected per head via `MODEL_FAMILY_1X2`/`_OU`/`_DRAW` (default `xgboost`, **verified byte-identical** to the pre-seam path by diffing model files + prediction CSVs); training writes a `models/model_meta_<market>.json` sidecar that serving reads (legacy XGBoost-JSON fallback when absent). Registered families: 1X2 `xgboost`/`logreg`/`rf`; O/U `xgboost`/`poisson_glm`; draw `xgboost`/`logreg`. **Why it matters here**: D3 (Dixon-Coles) and any non-XGBoost challenger can now be trained + A/B-tested through `benchmark_models.py` and served without pipeline surgery. End-to-end smoke test (logreg 1X2 challenger, 2026-06-23): trained → served → predicted, agreed with XGBoost on all 6 picks (mean |ΔP(home)| ≈ 1pt), with the XGBoost O/U head still getting correct `league_cat` handling via the `known_leagues` fallback. **Caveat**: swapping the production model invalidates `league_calibration.json` — route real swaps through `bin/retrain_pipeline.sh`.
